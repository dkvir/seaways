...
export class Gizmos {
  ...
  drawSphere(
    ...
    position,
    size = 1.0,
    color = vec4.fromValues(0.5, 1.0, 0.5, 0.0)
  ) {
    this.drawInternal(
      camera,
  ...
  drawPatch(camera, patch, yOffset = 0.1) {
  ...
  drawAxes(
    ...
    size = 1.0,
    color = vec4.fromValues(1.0, 1.0, 1.0, 0.0)
  ) {
  ...
  drawGeometry(
    ...
    rotation = quat.create(),
    size = 1.0,
    color = vec4.fromValues(0.75, 0.75, 0.75, 1.0)
  ) {
    this.drawInternal(
      camera,
      geometry,
  ...
  drawFloatingBody(camera, body, boyancy, geometry) {
  ...
  drawInternal(camera, geometry, color, position, rotation, scaling) {
...
const createPatchMesh = (resolution) => {
  const indices = [];
  const points = [];
  const colors = [];
...
const createWiredSphereMesh = (subdivisions = 32) => {
  let phi = 0.0;
...
const createXZCircleMesh = (subdivisions = 32) => {
  let phi = 0.0;
  const dPhi = (Math.PI * 2.0) / subdivisions;
