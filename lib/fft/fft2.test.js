...
import { abs, add, mult, complex, sub, eix, conj, im } from "./complex";
...
export const testDft2 = () => {
  for (let pow of [1, 2, 3, 4, 5, 6, 7, 8]) {
    // Arrange
    const size = 1 << pow;
    const signal = [...Array(size).keys()].map(() =>
      [...Array(size).keys()]
    ...
    const closeEnough = diff.every((v) => v <= 1.0e-5);
    if (!closeEnough) {
      console.warn("testDft2: Test didn't pass: ", diff);
      return;
    }
  }
...
export const testFft2 = () => {
  for (let pow of [1, 2, 3, 4, 5, 6, 7, 8]) {
    ...
    const closeEnough = diff.every((v) => v <= 1.0e-5);
    if (!closeEnough) {
      console.warn("testFft2: Test didn't pass: ", diff);
      return;
    }
  }
...
export const testFft2Combined = () => {
  ...
  for (let pow of [1, 2, 3, 4, 5, 6, 7, 8]) {
    // Arrange
    const size = 1 << pow;
    const signal0 = [...Array(size).keys()].map(() =>
      [...Array(size).keys()]
        .map(() => Math.random() * 2.0 - 1.0)
        .map((v) => complex(v, 0.0))
    );
    const signal1 = [...Array(size).keys()].map(() =>
      [...Array(size).keys()]
        .map(() => Math.random() * 2.0 - 1.0)
        .map((v) => complex(v, 0.0))
    );
    ...
    const closeEnough = diff.every((v) => v <= 1.0e-5);
    if (!closeEnough) {
      console.warn("testFft2Combined: Test didn't pass: ", diff);
      return;
    }
  }
...
export const testFft2Hermitian = () => {
  for (let pow of [1, 2, 3, 4, 5, 6, 7, 8]) {
    ...
    if (!isReal) {
      console.warn(`testFft2Hermitian: Test didn't pass`);
      return;
    }
  }
