...
import { abs, add, complex, mult, scale, sub, ifft } from "../fft";
...
export const testButterflyTexture = () => {
  for (let pow of [1, 2, 3, 4, 5, 6, 7, 8]) {
    ...
    const pingPong = [[...signal], new Array(size)];
    ...
    // Assert
    const actual = pingPong[src];
    const expected = ifft(signal);

    const diff = actual.map((a, i) => abs(sub(a, expected[i])));
    const closeEnough = diff.every((v) => v <= 1.0e-5);
    if (!closeEnough) {
      console.warn("testButterflyTexture: Test didn't pass: ", diff);
      return;
    }
  }
