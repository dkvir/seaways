...
export const testOceanFieldIfft2 = () => {
  ...
  const buffer = new Float32Array(
    oceanField.params.resolution * oceanField.params.resolution * 4
  );
  ...
  for (let slot of [0, 1, 2, 3, 4, 5]) {
    for (let couple of [0, 1]) {
      ...
      const expected = ifft2(
        float4ToComplex2d(buffer, oceanField.params.resolution, couple * 2)
      ).flat(1);

      // Act
      oceanField["ifft2"]();
      gpu.attachTexture(framebuffer, oceanField["ifftTextures"][slot], 0);
      gpu.readValues(
        framebuffer,
        buffer,
        oceanField.params.resolution,
      ...
      if (!closeEnough) {
        console.warn(
          `testOceanFieldIfft2 [slot ${slot}-${couple}]: Test didn't pass: `,
          diff
        );
        return;
      }
